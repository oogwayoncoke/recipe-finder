...
export default function AuthForm({ initialMode = 'login' }) {
    ...
    return (
      <Page>
        ...
        <div style={cardStyle}>
          ...
          <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: '1.25rem', color: 'var(--text)', marginBottom: '0.5rem' }}>Check your email</h2>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-dim)', fontWeight: 300, lineHeight: 1.6, marginBottom: '1.25rem' }}>
            We sent a verification link to{' '}
            <span style={{ color: 'var(--text-muted)' }}>{fields.email}</span>.
          </p>
          <button onClick={() => switchMode('login')} style={ghostBtnStyle}>Back to login →</button>
        </div>
      </Page>
    )
  }
      ...
      <Page>
        ...
        {forgotSent ? (
          <div style={{ marginTop: '2rem', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: 'var(--text-dim)', fontWeight: 300, lineHeight: 1.6, marginBottom: '1.5rem' }}>
              If an account exists for{' '}
              <span style={{ color: 'var(--text-muted)' }}>{fields.email}</span>,
              we sent a reset link. Check your spam too.
            </p>
          ...
          <>
            ...
            <button onClick={() => switchMode('login')} style={{ ...ghostBtnStyle, marginTop: '1rem' }}>
              ← back to login
            </button>
          </>
        )}
  ...
  return (
    <Page>
      <Logo />
      <p style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.68rem', color: 'var(--text-dim)', letterSpacing: '0.1em', marginBottom: '2rem' }}>
      ...
      <button
        ...
        onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border-2)'; e.currentTarget.style.backgroundColor = 'var(--bg-hover)' }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.backgroundColor = 'var(--bg-card)' }}
      >
        {googleLoading ? (
          <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.75rem', color: 'var(--text-dim)' }}>connecting…</span>
        ) : (
          <>
            <GoogleIcon />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
        ...
        <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', color: 'var(--text-dim)', letterSpacing: '0.08em' }}>or</span>
        <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border)' }} />
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <button type="submit" disabled={submitting} style={{
          ...
          fontFamily: '"DM Mono", monospace', fontSize: '0.8rem', fontWeight: 500,
          letterSpacing: '0.06em', padding: '0.75rem',
          borderRadius: '0.375rem', border: 'none', cursor: submitting ? 'not-allowed' : 'pointer',
          opacity: submitting ? 0.4 : 1, transition: 'opacity 0.15s',
        }}>
      ...
      {mode === 'login' && (
        <button
          ...
          style={{ ...ghostBtnStyle, marginTop: '1rem' }}
        >
          Forgot password?
        </button>
      )}
...
function Page({ children }) {
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
...
function Field({ label, id, type, value, onChange, error, autoComplete }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      ...
      />
      {error && <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '0.65rem', color: 'var(--error)', letterSpacing: '0.05em' }}>{error}</span>}
    </div>
  )
}
...
const cardStyle = {
  ...
  borderRadius: '0.375rem', padding: '2rem', textAlign: 'center', marginTop: '2rem',
}
